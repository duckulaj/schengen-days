package com.example.schengen.engine;
...
public final class SchengenEngine {
    ...
    public record Stay(LocalDate entry, LocalDate exit) {
        public Stay {
            if (entry == null || exit == null) throw new IllegalArgumentException("Dates required");
            if (exit.isBefore(entry)) throw new IllegalArgumentException("exit before entry");
        }
    }
    ...
    public static final class MergedStays {
        ...
        private MergedStays(LocalDate[] starts, LocalDate[] ends, long[] prefixDays) {
            this.starts = starts; this.ends = ends; this.prefixDays = prefixDays;
        }

        public long overlapDays(LocalDate winStart, LocalDate winEnd) {
            if (starts.length == 0 || winEnd.isBefore(winStart)) return 0;
            ...
            if (winStart.isAfter(starts[l])) total -= ChronoUnit.DAYS.between(starts[l], winStart);
            if (winEnd.isBefore(ends[r])) total -= ChronoUnit.DAYS.between(winEnd, ends[r]);
        ...
        private int firstIntervalWithEndGte(LocalDate d) {
            ...
            while (lo <= hi) {
                int mid = (lo + hi) >>> 1;
                if (!ends[mid].isBefore(d)) { ans = mid; hi = mid - 1; }
                else lo = mid + 1;
            }
            return ans;
        }
        ...
        private int lastIntervalWithStartLte(LocalDate d) {
            ...
            while (lo <= hi) {
                int mid = (lo + hi) >>> 1;
                if (!starts[mid].isAfter(d)) { ans = mid; lo = mid + 1; }
                else hi = mid - 1;
            }
            return ans;
        }
    ...
    public static MergedStays preprocess(List<Stay> stays) {
        if (stays == null || stays.isEmpty()) return new MergedStays(new LocalDate[0], new LocalDate[0], new long[0]);
        ...
        for (int i = 1; i < sorted.size(); i++) {
            ...
            if (!s.entry().isAfter(curE.plusDays(1))) {
                if (s.exit().isAfter(curE)) curE = s.exit();
            } else {
                ms.add(curS); me.add(curE);
                curS = s.entry(); curE = s.exit();
            }
        }
        ms.add(curS); me.add(curE);
    ...
    public static LocalDate nextLegalEntryDate(MergedStays merged, LocalDate referenceDate) {
        ...
        while (!d.isAfter(max)) {
            if (usedDaysLast180(merged, d) <= 89) return d;
            d = d.plusDays(1);
        }
